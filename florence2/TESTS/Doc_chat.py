...
# Chat input: Get user prompt and generate response
...
# KO Generation: Triggered by a button
if st.button("Generate Knowledge Object (KO)"):
    prompt="generate a KO article based off the context"
    doc_context = bm25s_search(prompt, st.session_state.bm25s_retriever, st.session_state.bm25s_stemmer, st.session_state.bm25s_corpus)
    with st.spinner("Generating KO..."):
        if len(doc_context) == 0:
            st.warning("Please ask a question related to the document before generating the KO.")
        else:
            ko_response = generate_ko(doc_context, prompt)
            message(ko_response)
